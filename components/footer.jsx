  ...
  return (
    <div className="flex flex-col items-center justify-evenly rounded-tl-[80px] py-8 text-center  md:flex-row md:items-start md:justify-around" style={{backgroundColor: 'var(--very-dark-gray-blue)',color: 'var(--footer-text)'}}>
      ...
      <div className="flex flex-col my-4">
        ...
        {productMenu.map(item=> <a key={item.id} className="ubuntu font-extralight hover:cursor-pointer">{item.label}</a>)}
      </div>

      <div className="flex flex-col my-4">
        <p className="mb-2 text-white">Company</p>
        {companyMenu.map(item=> <a key={item.id} className="ubuntu font-extralight hover:cursor-pointer">{item.label}</a>)}
        
      </div>

      <div className="flex flex-col my-4">
        <p className="mb-2 text-white">Connect</p>
        {connectMenu.map(item=> <a key={item.id} className="ubuntu font-extralight hover:cursor-pointer">{item.label}</a>)}
      </div>
    </div>
  );
}
